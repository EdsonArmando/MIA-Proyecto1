...
func EjecutarComandoReporte(nombreComando string,propiedadesTemp []Propiedad,ListaDiscos *list.List)(ParamValidos bool){
	...
	if len(propiedadesTemp) >= 1{
        ...
        case "mbr":
            GraficarMBR_EBR(propiedades[0],ListaDiscos)
        case "disk":
            GraficarDisco(propiedades[0],ListaDiscos)
        case "sb":
            GraficarSuperBloque(propiedades[0],ListaDiscos)
        case "bm_arbdir":
        ...
        case "bitacora":
            Reporte_bitacora(propiedades[0],propiedades[1],propiedades[3],ListaDiscos)
        case "directorio":
...
func Reporte_bitacora(idParticion string,pathCarpeta string,ruta string,ListaDiscos *list.List)(bool){
    return false
}
...
func Reporte(idParticion string,ListaDiscos *list.List)(bool){
    ...
    fmt.Println("---------------Bitmap Bloque------------------------",cont)
...
func GraficarMBR_EBR(idParticion string,ListaDiscos *list.List)(bool){
    ...
    var datos string
    datos = string(buffer.String())
    CreateArchivo("/home/edson/Escritorio/Proyecto/Proyecto1/MBR_EBR.dot",datos)
    return false
}
//Graficar Disco y calcular Porcentajes
func GraficarDisco(idParticion string,ListaDiscos *list.List)(bool){
    var NombreParticion [15]byte
    var buffer bytes.Buffer
    ...
    var datos string
    datos = string(buffer.String())
    CreateArchivo("/home/edson/Escritorio/Proyecto/Proyecto1/Disco.dot",datos)
    return false
}
func GraficarSuperBloque(idParticion string,ListaDiscos *list.List)(bool){
    sb:=SB{}
    pathDisco,nombreParticion,_:=RecorrerListaDisco(idParticion,ListaDiscos)
    sb,_= DevolverSuperBlque(pathDisco,nombreParticion)
    var buffer bytes.Buffer
    ...
    buffer.WriteString("</table>\n>];\n}\n}")
    fmt.Println(sb.ConteoAVD,sb.ConteoDD,sb.ConteoInodo,sb.ConteoBloque)
    var datos string
    datos = string(buffer.String())
    CreateArchivo("/home/edson/Escritorio/Proyecto/Proyecto1/SB.dot",datos)
    return false
    return false
}
...
func CreateArchivo(path string,data string){
    ...
    executeComand("dot -Tpdf "+ nombreArchivo +" -o "+ nombreArchivo[0:len(nombreArchivo)-4] +".pdf")
}	
